...
`moderate` and `severe` appear to be parallel, while mild seems to be much more elastic (flatter) than the other two. Due to mild having a distinct slope, we can expect an interaction, and it would produce a significant effect for `log_days`.
...
At $\alpha = 0.10$, all the differences are significant for both explanatory variables (since all p-values are less than $\alpha$). That is, any combination of `duration` and `weight gain` has a unique allocation of `log_days`.
